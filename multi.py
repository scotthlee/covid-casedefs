...
def simulate_coverage(pvec, 
                      ...
                      dep=False,
                      bonf=False):
    # Generating samples from each of the distributions, assuming independence
    if bonf:
        bonf_a = a / len(pvec)
        input = [(samp_size, pvec, n_boot, bonf_a, dep)] * n_sims
    else:
        input = [(samp_size, pvec, n_boot, a, dep)] * n_sims
    p = Pool()
    p_out = p.starmap(tools.sample_coverage, input)
    sims = np.concatenate([arr.reshape(1, -1) for arr in p_out], axis=0)
    p.close()
    p.join()
    ...
    # Return coverage probs from the joint and marginal assumptions
    j_sums = np.sum(sims, axis=1)
    j_fam = np.mean(j_sums == len(pvec))
    
    cover = np.mean(sims, axis=0)
    fam = np.mean(cover)
